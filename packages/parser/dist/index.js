    ...
    for (let idx = 0; idx < lines.length; idx++) {
        ...
        if (ln.startsWith('fn ')) {
            ...
            if (m) {
                ...
                const openCount = (s) => (s.match(/\{/g) || []).length;
                const closeCount = (s) => (s.match(/\}/g) || []).length;
                ...
                if (/^match\b/.test(bodySrc) && (bodySrc.split('{').length - 1) > (bodySrc.split('}').length - 1)) {
                    ...
                    while (depth2 > 0 && idx + 1 < lines.length) {
                        idx++;
                        bodySrc += '\n' + lines[idx];
                        depth2 += (lines[idx].match(/\{/g) || []).length - (lines[idx].match(/\}/g) || []).length;
                    }
                }
